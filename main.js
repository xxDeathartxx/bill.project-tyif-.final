...
onValueselect=()=>{
 ...
 Pdel=(id)=>{
    ...
    checkbill=()=>{
    let table=document.getElementById('bill_body').rows;
   let billinfo=[];
   let total=0; 
   for(let i=0;i<table.length;i++){

        let product=table[i].cells[1].innerText;
        let quantity=table[i].cells[2].innerText;
        let productinfo=product+'|'+quantity;
        billinfo.push(productinfo);
        total=total+parseInt(table[i].cells[4].innerText);
 
    }
        let body={billinfo:billinfo,total:total};
        let paramters=JSON.stringify(body);
        const url='/transactions/add';
       const xhr=new XMLHttpRequest();
